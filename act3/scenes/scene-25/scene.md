# Scene 25 - Compliance Ping / Ada's Screen

## Purpose
Materialize consequences: Ada's rule-breaking is now tracked.

## Target Runtime
~1m20s-1m50s

---

## Story Beat (What happens)
Automated compliance notice appears; review is scheduled, making cost institutional.

Ada reads it twice. On the third look she scrolls to the routing metadata below the notice body - the part most people don't read. The flag wasn't just triggered; it was reviewed. Someone above Marcus already saw this ticket 36 hours before his calendar invite appeared. They escalated it downward. They chose the path.

She doesn't react visibly. She minimizes the alert and keeps working.

The distinction matters: she thought nobody had looked. Now she knows someone did.

---

## Setting & Environment
Desk/admin console context with security notifications visible.

---

## Visual Design
- Keep realism-forward cinematography; avoid stylized sci-fi gloss
- Use contrast between routine operations and emotional gravity
- Emphasize readable text/object inserts on decision points
- Preserve continuity in palette, wardrobe, and prop state

---

## Set Dressing / Objects (must-have)
- Functional monitors with legible UI/text
- Continuity props for Ada (mug, badge, notebook, carry items)
- Scene-specific anchor objects (forms, compliance notices, thermos, rollout checklist)
- Chronology markers when time pressure matters

---

## Sound Design
UI alert tone + office ambience; keep it dry.

Mix rule: reduce ambience on revelation lines; restore full environmental tone immediately after.

---

## Performance Direction
- Restrained, specific performances; avoid overt melodrama
- Emotional shifts should register in timing and breath before dialogue
- Keep procedural dialogue casual and believable
- Let grief appear through choices, not speeches

---

## Blocking
1. Enter in active task/state.
2. Deliver the scene's pivotal information or choice.
3. Hold reaction beat with controlled stillness.
4. Exit on forward motion into next consequence.

---

## Shot List (practical draft)
1. Establishing frame (context + routine) - 4-8s
2. Functional coverage for action/dialogue progression
3. Insert(s) on critical text/object cues
4. Reaction close-up on turning beat
5. Exit/hold frame that carries narrative pressure onward

---

## Dialogue / On-screen Text (key lines)
# Scene 25 — Compliance Ping / Ada's Screen

Automated alert:

**Policy Notice: Elevated access anomaly detected**  
**Review scheduled with manager (Marcus), Wednesday 10:00**

Ada reads it.
Then scrolls down to the routing metadata — the part that looks like system noise:

```
ROUTING CHAIN
auto-flag triggered: 2026-02-17 08:14
reviewed: Director, Infrastructure Operations [D. Reyes] — 2026-02-17 09:02
escalated to: supervisor (Marcus) — 2026-02-17 11:48
review scheduled: Wednesday 10:00
```

Ada stares at the timestamp.

Not triggered-and-forgotten. Reviewed. By someone who had the authority to stop it.
Thirty-six hours before Marcus got the calendar invite, someone looked at this ticket, decided it was worth a Wednesday conversation rather than immediate closure, and moved on.

She doesn't move.

She opens a second tab. Navigates to the archive directory.
Cursor lands on the delete command.
Her finger is on the key.

Three seconds.

She closes the tab without typing.

Then minimizes the alert and keeps working.

The cost has found her name. And someone above it already signed.
This is the moment "witness" stops being instinct and becomes choice.

---

---

## Why this scene matters
This scene is structurally necessary to Act III: it delivers catastrophe progression, paid cost, or earned transformation without filler.

## Narrative Sync (Latest Act III Draft)

### Scene Core
Automated alert:

**Policy Notice: Elevated access anomaly detected**
**Review scheduled with manager (Marcus), Wednesday 10:00**

Ada reads it twice.
Then minimizes it and keeps working.

The cost has found her name.

---

<!-- VO_TIMING_START -->
## VO / Dialogue Timing (Embedded)

_Source: `production/vo-script.md`, Scene 25. Runtime target: 1:20–1:50_

| TC | Speaker | Line | Delivery |
|----|---------|------|----------|
| ~0:20–0:40 | ON-SCREEN | "Policy Notice: Elevated access anomaly detected / Review scheduled with manager (Marcus), Wednesday 10:00" | alert display; Ada reads twice |
| ~0:45–1:00 | ON-SCREEN | ROUTING CHAIN metadata (director-level review visible) | Ada scrolls; reads timestamp |
<!-- VO_TIMING_END -->

