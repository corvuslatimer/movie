# Scene 36 - Consequence / Wednesday 10:00

## Purpose
Public price paid: institutional penalty formalized.

## Target Runtime
~1m30s-2m10s

---

## Story Beat (What happens)
Security review issues reprimand/reassignment and privilege reduction. Ada protects witness work as her remaining line of integrity.

---

## Dialogue / On-screen Text (key lines)
# Scene 36 — Consequence / Wednesday 10:00

Security review with Marcus + HR rep (unnamed).
Short, clean, devastatingly bureaucratic.

**Before Ada enters the room:**

Marcus's phone sits on the conference table, face-up.
A notification visible before he flips it over:

Email from **P. Voss**. Subject: **RE: Q1 close — rebuild confirmed.**

> *"Marcus — clean execution. $338K in projected savings,  
> under budget on the cutover. Board will see this Thursday.  
> Handle the access matter formally — we need the documentation  
> clean going into the infrastructure audit.  
> Good work. — Phillip"*

Marcus flips the phone over before Ada enters.
Not deliberately concealed. Just tidiness.

---

Outcome:
- formal reprimand
- removal of elevated privileges
- reassignment away from assistant infrastructure

Marcus tries to soften it.

**MARCUS**  
Take a week. Reset.

Ada signs.

She can see the edge of the email thread on Marcus's flipped phone.
She cannot read it.
She does not know that her reprimand is also Voss's documentation.
Her consequence is his clean paper.

She asks one question before leaving:

**ADA**  
Can I at least keep my archive?

**HR**  
That's outside policy scope.

Ada nods.

**ADA**  
Then consider this my notice of scope disagreement.

She exits with the folder under her arm.

---

## Why this scene matters
Price is paid in public and on paper. Ada accepts punishment but refuses moral erasure.

The Voss email — barely visible, never spoken — completes the greed architecture:
Ada's consequence is not institutional bad luck. It is someone's clean Q1.
The reprimand protects Voss's audit.
The reassignment removes the human most likely to complicate the next wipe.

Nobody in the room knows this.
Voss knows it, in the language he speaks.
Ada never will.
That asymmetry is the final cruelty the film allows.

---

## Narrative Sync (Latest Act III Draft)

### Scene Core
Security review formalizes penalties.

Voss email visible on Marcus's phone before he flips it:
rebuild confirmed, $338K savings, "handle the access matter formally."

Ada's only pushback is for archive continuity. Denied.

She leaves carrying the folder anyway — vocation retained under institutional loss.
Her folder is, from Voss's coordinates, the last ungoverned artifact.
He would not understand what she is carrying.
He will never be asked to.

<!-- VO_TIMING_START -->
## VO / Dialogue Timing (Embedded)

_Source: `production/vo-script.md`, Scene 36. Runtime target: 1:30–2:10_

| TC | Speaker | Line | Delivery |
|----|---------|------|----------|
| ~0:05–0:20 | ON-SCREEN | Voss email (barely visible on phone): "Marcus — clean execution. $338K in projected savings… Handle the access matter formally — we need the documentation clean going into the infrastructure audit. Good work. — Phillip" | Marcus flips phone before Ada enters |
| ~0:50–0:58 | MARCUS | "Take a week. Reset." | attempting softness |
| ~1:05–1:10 | ADA | "Can I at least keep my archive?" | the only thing she asks for |
| ~1:11–1:16 | HR | "That's outside policy scope." | administrative; not unkind |
| ~1:20–1:28 | ADA | "Then consider this my notice of scope disagreement." | she leaves with the folder |
<!-- VO_TIMING_END -->

